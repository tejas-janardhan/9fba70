...
class Conversations(APIView):
    ...
    def put(self, request: Request):
        ...
        if conversation.user1 and conversation.user1.id == user_id:
            last_read_message = conversation.messages.filter(senderId=conversation.user2.id).last()
            conversation.last_read_message_user1 = last_read_message
        elif conversation.user2 and conversation.user2.id == user_id:
            last_read_message = conversation.messages.filter(senderId=conversation.user1.id).last()
            conversation.last_read_message_user2 = last_read_message

        response_dict = {'lastReadMessageOtherUser': Message.get_none_or_dict(last_read_message)}
        conversation.save()
    ...
    def get(self, request: Request):
        ...
        for convo in conversations:
            convo_dict = {
                "id": convo.id,
                "messages": [
                    message.to_dict(message_fields)
            ...
            if convo.user1 and convo.user1.id != user_id:
                ...
                convo_dict['lastReadMessageOtherUser'] = Message.get_none_or_dict(convo.last_read_message_user1,
                                                                                  message_fields)
                ...
                convo_dict['lastReadMessageOtherUser'] = Message.get_none_or_dict(convo.last_read_message_user2,
                                                                                  message_fields)
            ...
            convo_dict['unreadMessageCount'] = Conversation.get_unread_message_count(convo_dict["messages"],
                                                                                     last_read_message,
